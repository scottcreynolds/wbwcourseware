...
const syncableCohorts=computed(()=>cohorts.value.filter(cohort=>cohort.status!=='archived'))
...
async function load(): Promise<void> {
  ...
  try {
    ...
    syncSelection.value = syncableCohorts.value.map(cohort => cohort.id)
  } catch {
...
async function syncToCohorts(){if(!syncSelection.value.length)return;try{const count=await cohortService.syncItem(itemId,syncSelection.value);notice.value=`Updated ${count} cohort item${count===1?'':'s'}.`}catch{errorMessage.value='Course change could not be applied to selected cohorts.'}}
...
<template>
  ...
  <template v-else-if="item">
    ...
    <v-card v-if="syncableCohorts.length" border class="mt-6"><v-card-title>Apply changes to cohorts</v-card-title><v-card-text><p class="mb-3">Overwrites matching content and resources. Cohort due dates, releases, and publication state remain unchanged. All active and draft cohorts are selected by default; uncheck any you want to leave untouched.</p><v-select v-model="syncSelection" :items="syncableCohorts" item-title="title" item-value="id" label="Cohorts" multiple chips /></v-card-text><v-card-actions><v-btn color="primary" :disabled="!syncSelection.length" @click="syncToCohorts">Apply changes</v-btn></v-card-actions></v-card>
  </template>
