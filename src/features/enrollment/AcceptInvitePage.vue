...
<template><v-card class="auth-card" border><v-card-text><v-alert v-if="message" type="error" class="mb-4">{{ message }}</v-alert><v-alert v-if="complete" type="success">Invitation accepted. Sign in to open your cohort.</v-alert><v-form v-else @submit.prevent="accept"><template v-if="!auth.isAuthenticated"><v-text-field v-model="displayName" label="Name" autocomplete="name" /><v-text-field v-model="password" label="Password" type="password" autocomplete="new-password" hint="At least 12 characters" /><v-text-field v-model="confirmation" label="Confirm password" type="password" autocomplete="new-password" /></template><p v-else class="mb-4">Connect this invitation to your signed-in account.</p><v-btn type="submit" color="primary" :loading="loading" :disabled="!token">Accept invitation</v-btn></v-form></v-card-text><v-card-actions v-if="complete"><v-btn to="/login">Continue to sign in</v-btn></v-card-actions></v-card></template>
