...
async function refresh(){try{const [r,inv,enr]=await Promise.all([cohortService.load(id),enrollmentService.listInvitations(id),enrollmentService.listEnrollments(id)]);cohort.value=r.cohort;modules.value=r.modules;items.value=r.items;invitations.value=inv;enrollments.value=enr}catch{message.value='Cohort could not be loaded.'}}
async function load(){loading.value=true;await refresh();loading.value=false}
async function run(action:()=>Promise<void>,text:string){saving.value=true;message.value=null;try{await action();message.value=text;await refresh()}catch(e){message.value=e instanceof Error?e.message:'Change could not be saved.'}finally{saving.value=false}}
...
<template>
  ...
  <template v-else-if="cohort">
    ...
    <v-tabs v-model="activeTab" class="mb-4">
      <v-tab value="curriculum">Curriculum</v-tab>
    ...
    <v-window v-model="activeTab">
      ...
      <v-window-item value="announcements">
        <AnnouncementList :cohort-id="id" teacher />
      </v-window-item>
      <v-window-item value="discussions">
        <DiscussionBoard :cohort-id="id" teacher />
      </v-window-item>
    </v-window>
  </template>
</template>
